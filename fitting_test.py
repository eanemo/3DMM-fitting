...
cv2.imwrite("texture.png", frontal_img)
...
affine_camera_matrix = RenderingParameters.get_3x4_affine_camera_matrix(frontal_rendering_params, width, height)

texture = render.extract_tecture(py_mesh, affine_camera_matrix, frontal_img)
uv = render.compute_vertice_uv(frontal_img, py_mesh, frontal_rendering_params.get_modelview(), frontal_rendering_params.get_projection(),
    RenderingParameters.get_opencv_viewport(width * s, height * s), scale_param / s)

render.save_ply(py_mesh, frontal_pic_name + '-output', [210, 183, 108], author='Yinghao Li')
render.save_ply_texture(py_mesh, frontal_pic_name + 'texture-output', textureFile="texture.png", uvList=uv, author='Nemotec')
cv2.imwrite("isomap.jpg", texture)